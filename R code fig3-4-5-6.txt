...
ecrix1 = zoo(ecrix, order.by = index(crix))
efcrix1 = zoo(efcrix, order.by = index(crix))
...
plot.zoo(crix, plot.type = "multiple", type = "l", lwd = 1.5, panel = my.panel, 
...
rix = zoo(cbind(cret, ecret, efcret), order.by = index(crix))
...
rixres = zoo(cbind(cres, ecres, efcres), order.by = index(crix))
...
par(mfrow = c(1, 1))
serr = zoo(fitall@mfit$stdresid, order.by = index(crix))
...
par(mfrow = c(3, 1))
plot(zoo(rcorr[1, 2, ], order.by = index(crix)), lwd = 2, ylab = NA, xlab = NA, 
     main = "CRIX v.s. ECRIX")
plot(zoo(rcorr[1, 3, ], order.by = index(crix)), lwd = 2, ylab = NA, xlab = NA, 
     main = "CRIX v.s. EFCRIX")
plot(zoo(rcorr[2, 3, ], order.by = index(crix)), lwd = 2, ylab = NA, xlab = NA, 
